...
/*   Updated: 2023/12/23 17:33:35 by eel-brah         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int is_area_valid(t_stack *a, t_sorted area)
{
...
int *gen_area(t_stack *a, int *area_size)
{
	t_node *a_stack;

	t_sorted area = find_the_largest_sorted_area(a);
	// t_node *a_stack;
	int *sorted_area = NULL;
	int valid = is_area_valid(a, area);
	if (valid)
	{
		*area_size = area.size;
		sorted_area = malloc(area.size * sizeof(int));
		a_stack = a->head;
		int poz = 0;
		while (poz < area.begin_poz)
		{
			poz++;
			a_stack = a_stack->next;
		}
...
int	send_to_b(t_stack *a, t_stack *b, int *array, int size)
{
	int poz;
	int *begin;
	int a_size;
	...
	int r = 0;
	int area_size = 0;
	int *sorted_area = gen_area(a, &area_size);
	if (area_size == 0)
		area_size = 3;
	if (area_size == 3)
		r = 1;
	// ft_printf("%s", sorted_area);
	a_stack = a->head;
// ft_printf("%i \n", sorted_area[0]);
// return ;
	poz = 0;
	...
	while ((int)b->size < a_size)
	{
		...
		while (a_stack && i < size)
		{
			...
			{
				if (!(sorted_area && is_exixt(a_stack->item, sorted_area, area_size)))
				{
